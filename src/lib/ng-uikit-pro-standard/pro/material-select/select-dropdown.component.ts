  ...
  implements AfterViewInit, OnChanges, OnInit
{
  ...
  @HostListener("keyup") onkeyup() {
    this.hasOptionsItems =
      this._elementRef.nativeElement.childNodes[0].children[1].children[0]
        .children.length >= 1
        ? true
        : false;
  }
  ...
  ngAfterViewInit() {
    ...
    if (this.multiple) {
      this._elementRef.nativeElement
        .querySelectorAll(".disabled.optgroup")
        .forEach((element: any) => {
          this._renderer.setStyle(
    ...
    try {
      if (!(this._elementRef.nativeElement.parentElement == undefined)) {
        setTimeout(() => {
          if (
            this._elementRef.nativeElement.parentElement.attributes
              .customClass !== undefined
          ) {
            this.customClass =
              this._elementRef.nativeElement.parentElement.attributes.customClass.value;
          }
        }, 0);
      }
    } catch (error) {}
