    ...
    AfterContentChecked
{
  ...
  public dayIdx = 0;
  public weekDayOpts: Array<string> = [
    "su",
    "mo",
    "tu",
    "we",
  ...
  removeInlineStyle() {
    try {
      if (
        this.elem.nativeElement.parentElement.parentElement.classList.contains(
          "modal-content"
        )
      ) {
  ...
  addLocale(locale: IMyLocales) {
    this.localeService.locales = Object.assign(
      {},
  ...
  onUserMonthInput(value: string): void {
    this.invalidMonth = false;
    const m: number = this.utilService.isMonthLabelValid(
      value,
      this.opts.monthLabels
    );
    if (m !== -1) {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (
      changes.hasOwnProperty("selector") &&
      changes["selector"].currentValue > 0
    ) {
      this.openBtnClicked();
    }
    ...
    try {
      if (
        ...
        if (this.firstTimeOpenedModal) {
          this.modalHeightBefore =
            this.elem.nativeElement.parentElement.parentElement.offsetHeight;
        }
        ...
        this.elem.nativeElement.parentElement.parentElement.style.height =
          this.modalHeightBefore +
          this.pickerFrame.nativeElement.offsetHeight +
          "px";
      }
    } catch (error) {}
  ...
  todayClicked(): void {
    ...
    if (
      today.year !== this.visibleMonth.year ||
  ...
  formatDate(val: any): string {
    ...
    const yy =
      val.year.toString().length === 2
        ? val.year
        : val.year.toString().slice(2, 4); // 00 - 99
    const yyyy = val.year;
  ...
  parseSelectedDate(selDate: any): IMyDate {
    ...
    if (typeof selDate === "string") {
      ...
      date.day = this.utilService.parseDatePartNumber(
        this.opts.dateFormat,
        sd,
        "dd"
      );
      ...
      date.month =
        this.opts.dateFormat.indexOf("mmm") !== -1
          ? this.utilService.parseDatePartMonthName(
              this.opts.dateFormat,
              sd,
              "mmm",
              this.opts.monthLabels
            )
          : this.utilService.parseDatePartNumber(
              this.opts.dateFormat,
              sd,
              "mm"
            );

      date.year = this.utilService.parseDatePartNumber(
        this.opts.dateFormat,
        sd,
        "yyyy"
      );
