  ...
  implements OnInit, AfterViewInit, ControlValueAccessor, AfterContentChecked
{
  ...
  mousedownEvent: any =
    "mousedown" + (this.touchSupported ? " touchstart" : "");
  ...
  @HostListener("touchmove", ["$event"]) ontouchmove(event: any) {
    ...
    if (event.target.parentElement.classList.contains("clockpicker-dial")) {
      (
        this.elem.nativeElement.querySelectorAll(".clockpicker-tick") as any
      ).forEach((element: any) => {
  ...
  mousedown(e: any, space?: any) {
    ...
    if (
      space &&
      (z < this.outerRadius - this.tickRadius ||
        z > this.outerRadius + this.tickRadius)
    ) {
      return;
    }
  ...
  generateTick() {
    ...
    for (let i = 0; i < 60; i += 5) {
      ...
      const tick = {
        ...
        left:
          this.dialRadius +
          Math.sin(radian) * this.outerRadius -
          this.tickRadius,
        top:
          this.dialRadius -
          Math.cos(radian) * this.outerRadius -
          this.tickRadius,
      };
  ...
  setHand(x: any, y: any, roundBy5: any) {
    ...
    if (isHours) {
      ...
      if (value % 5 === 0) {
        ...
        this.fg.nativeElement.setAttribute(
          "class",
