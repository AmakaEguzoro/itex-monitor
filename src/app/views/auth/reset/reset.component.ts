...
export class ResetComponent implements OnInit {
  ...
  validateInput(c: FormControl) {
    ...
    // return (emailFormat.test(c.value) || idFormat.test(c.value)) ? null : {
      return (idFormat.test(c.value)) ? null : {
      validateInput: {
        valid: false
      }
    };
  ...
  errorHandler() {
    ...
    this.errors = {
      email_detail: (!this.userDetails.get('email').valid && this.email) ? "This is not a Merchant ID" : "",
      email: !this.email ? "Merchant ID is required" : ""
    }
